...
public class ChunkTask extends Task {
    ...
    public void run() {
        ...
        if (this.peer.getStorage().hasStoredChunk(this.message.getFileId(), this.message.getChunkNo())) {
            ...
            chunk.setSent(true);
            System.out.println("Received CHUNK from  " + this.message.getSenderId() + " for chunk no " + this.message.getChunkNo());
        }
        ...
        // If it is a sent chunk, add body to the chunk so we can restore information
            ...
            System.out.println("Add body to chunk no " + this.message.getChunkNo());
        }
    }
}
